...
namespace client
{
    public partial class client : Form
    {
        ...
        private void butFWebcam_Click(object sender, EventArgs e)
        {
            // Kiểm tra kết nối 
            if (Program.client == null)
            {
            ...
            try
            {
                // 1. Gửi lệnh yêu cầu vào chế độ Webcam
                String s = "WEBCAM";
                Program.nw.WriteLine(s);
                Program.nw.Flush();

                // 2. Gửi thời gian muốn bật
                Program.nw.WriteLine("0");
                Program.nw.Flush();

                // 3. Mở form hiển thị Webcam
                FWebcam webcamForm = new FWebcam();
